...
import StakeListItem from '@/components/oldAdmin/StakeListItem';
...
const Admin = ({ web3, account }: { web3: any; account: string }) => {
    ...
    const burnHUT = async (): Promise<any> => {
        ...
        const { data } = await axios.post('https://server.efforthye.com/api/contract/burn', {
            amount: burnInput,
            account: web3.account,
        });
        if (web3.web3) {
    ...
    const getAdminTxFunc = async () => {
        try {
            const { data } = await axios.get('https://server.efforthye.com/api/admin/transactionList');
            setAdminTxList([...data]);
            return data;
        } catch (error) {
            console.error(error);
    ...
    const getStakingList = async () => {
        try {
            const { data } = await axios.get('https://server.efforthye.com/api/admin/stakingList');
    ...
    const updateDBDurationAndCharge = async () => {
        const { data } = await axios.post('https://server.efforthye.com/api/admin/updateDurationAndCharge', {
            stakeDurationList,
        });
        return data;
    };
