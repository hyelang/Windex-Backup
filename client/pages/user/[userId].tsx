...
const UserPage = () => {
    ...
    const requestImage = async () => {
        ...
        try {
            const { data: url } = await axios.post('https://server.efforthye.com/api/user/profileSrc', curImgFormData, {
...
export const getServerSideProps: GetServerSideProps = async () => {
    // 로그인 쿠키 요청
    try {
        const { data: login } = await axios.post('https://server.efforthye.com/api/user/login', {
            id: '1',
            pw: '1234',
            pw2: SHA256('12345'),
        });
