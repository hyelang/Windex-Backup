...
import { modalType } from '@/module/features/modalSlice';
...
const BatchContentsCard = ({
    ...
    data: undefined | [obj: IBatchContents];
    account: string;
    web3: any;
    durationData: number[];
}) => {
    ...
    const sendBatchData = async (data: batchData) => {
        let status = '';
        try {
            const batchData = (await axios.post('https://server.efforthye.com/api/contract/batchStaking', data)).data;
            ...
            if (web3.web3) {
                ...
                await axios.post('https://server.efforthye.com/api/staking/batchStakeSave', {
                    stakeList: data,
                ...
                return (
                    <BatchContentsBox key={`batchBox-${index}`}>
                        <div key={`nftName-${index}`}>
                            <div>
                                <PictureImg imgSrc={item.nft.img} size={'small'} />
                            </div>
                            <div>
                                {item.nft.name} <p>#{item.tokenId}</p>
                            </div>
                        </div>
                        <div key={`price-${index}`}>{item.price} HUT</div>
