...
const SwapBox = ({ setExchangePercent, setTokenExchange, web3, account }: SwapProps) => {
    ...
    const swapRequest = async () => {
        ...
        let status = '';
        try {
            const { data } = await axios.post('https://server.efforthye.com/api/contract/swap', {
                ...swapToken,
            ...
            if (data.buyObj) {
                if (web3.web3) {
                    try {
                        ...
                        const { data: saveData } = await axios.post('https://server.efforthye.com/api/swap/save', {
                            ...buy,
    ...
    const exchangeRequest = async () => {
        ...
        try {
            ...
            } = await axios.get('https://server.efforthye.com/api/swap/exchange', {
                params: {
                    ...swapCheckData,
                },
            });
                    ...
                    <SwapContent>
                        <div>
                            <input
                                ...
                                max={swapToken.from}
                                step="0.001"
                                disabled={swapToken.from == 'notChoose'}
                            />
                        </div>
                        <div>
                            <div>
