...
router.post('/batchStaking', async (req, res) => {
    ...
    const batchStake = await deployedS.methods
        .stakingBatch(req.body.tokenId, req.body.amount, req.body.duration, rewards)
        .encodeABI();
...
router.post('/withdrawMatic', async (req, res) => {
    ...
    const withdraw = await deployedV.methods.withdrawMatic(amount).encodeABI();
    console.log('해위');
...
router.post('/getFeeRate', async (req, res) => {
    const deployedV = new web3.eth.Contract(VendorAbi as AbiItem[], process.env.Vendor_CA);
    const feeRate = await deployedV.methods.getFeeRate().call();
    res.send(feeRate);
});
...
router.post('/IcoUserDisplay', async (req, res) => {
    ...
    const nowAmountToWei = await deployedC.methods.getNowAmountRaised().call();
